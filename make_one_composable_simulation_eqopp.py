...
# # Convert RuntimeWarnings into errors
...
def compute_eo(gAseen, gAacc, gBseen, gBacc):
...
def is_eo_smaller_or_equal_than_kappa(gAseen, gAacc, gBseen, gBacc, kappa):
    # if gAseen == 0 or gBseen == 0:
    #     return 0 <= kappa
...
def parse_args():
    ...
    parser.add_argument("--ml_model", type=str, default="experimental_results/ML_models/model_adult_sex_erm.pkl", help="Path to the trained ML model")
    ...
    parser.add_argument("--time_horizon", type=int, default=40, help="Time horizon for fairness properties")
    ...
    parser.add_argument("--n_time_windows", type=int, default=1, help="Number of time windows")
...
def load_shield_df(ml_model, dataset, sensitive_attr, time_horizon, n_cost_bins, dp_epsilon, cost_type, 
    ...
    shield_filepath = f"experimental_results/dp_enforcer_policies/eo_{base_filename}.txt"
    ...
    if not os.path.isfile(shield_filepath):
        ...
        make_cpp_input_from_ML_model.main(ml_model, dataset, sensitive_attr, time_horizon, n_cost_bins, dp_epsilon, cost_type, min_acc_rate=min_acc_rate, max_acc_rate=max_acc_rate, buff_gAacc=buff_gAacc, buff_gAseen=buff_gAseen, buff_gBacc=buff_gBacc, buff_gBseen=buff_gBseen,
         debug = debug, fairness_metric="eo")
...
def get_shield_value(df, rem_dec, gAseen, gAacc, gBseen, gBacc, prev_step, dp_threshold, 
    ...
    val = val_series.get((rem_dec, gAseen, gAacc, gBseen, gBacc, prev_step), -1)
    if val != -1:
        return val
    # dp = np.abs((gAacc+buff_gAacc)/(1.0+gAseen+buff_gAseen) - (gBacc+buff_gBacc)/(1.0+gBseen+buff_gBseen))
    # note that prev_step is not being taken into account here, in concordance with eo_enforcer.cc
    if is_eo_smaller_or_equal_than_kappa(gAseen+buff_gAseen, gAacc+buff_gAacc, gBseen+buff_gBseen, gBacc+buff_gBacc, dp_threshold):
    # if dp <= dp_threshold:
        return 0
    return np.inf
...
def make_long_window_simulation(net, ml_model, ml_algo, dataset, data_loader, sensitive_attr, 
    ...
    for step, (x, y, s) in enumerate(data_loader):
        ...
        log_gAseen0.append(gAseen0)
        log_gAacc0.append(gAacc0)
        log_gBseen0.append(gBseen0)
        log_gBacc0.append(gBacc0)   
...
def main(ml_model, dataset, sensitive_attr, time_horizon, n_cost_bins, dp_threshold, cost_type, lambda_decision, 
    ...
    res_df['dp'] = res_df.apply(lambda row: compute_eo(row['gAseen'], row['gAacc'], row['gBseen'], row['gBacc']), axis=1)
